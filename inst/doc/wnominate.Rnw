...
way to agriculture.  Keith Poole's VOTEVIEW software allows us to
...
can wee that Sparkman and Bartlett are the second and fifth
...
by a single dimension (ie. around 90$^\circ$), although there are a
...
2, showing that the vote largely broke down on strict partisan
lines.
